...
- **구글 드라이브 동기화** — 노트·녹음은 **항상 이 컴퓨터에 저장**되고, 드라이브에는 사본이 올라갑니다. 그래서 드라이브가 꺼져 있거나 인터넷이 없어도 앱은 평소대로 동작합니다. 홈 화면의 동기화 버튼으로 언제든 주고받을 수 있고, 세션을 마치거나 홈으로 돌아올 때 자동으로 올라갑니다. 다른 PC와 양쪽 다 바뀌었으면 어느 쪽을 쓸지 물어보고, 밀려나는 쪽은 `data/backups/`에 보관합니다. 용량 관리(녹음/모델 캐시 정리)도 설정에 있습니다.
